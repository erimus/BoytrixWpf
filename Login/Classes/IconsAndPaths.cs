...
namespace Boytrix.UI.WPF.BoytrixModules.Login.Classes
{
    public class IconsAndPaths
    {
                ...
                if ((fi.Extension == ".lnk"))
                {
                    ...
                    if ((potentialExe.Extension == ".exe"))
                    {
                        ...
                        try
                        {
                            ...
                            ico.ToBitmap().Save(tileIconPath, ImageFormat.Png);
        ...
        private void AddToDictionary(string displayName, string tileIconPath, string exePath)
        {
            if (!IconsPathsDi.ContainsKey(displayName))
            {
                IconsPathsDi.Add(displayName, new[] {
				tileIconPath,
				exePath
			});
            }
